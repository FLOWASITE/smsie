...
- **保活（nuôi SIM）**：預設關閉，且會產生費用 — 當某張 SIM 連續 `keepalive.interval_days` 天（預設 25；可用 `keepalive_interval` 針對單一 SIM 覆寫）沒有任何收發 SMS 時，smsie 會從該 SIM 發送一則付費的網內 SMS 給卡槽內同一電信商的另一張 SIM（絕不會發到卡槽以外），每張 SIM 每個日曆月最多 `keepalive.max_per_month` 次（預設 3）。每張 SIM 必須個別開啟（數據機設定的 `keepalive_enabled`，或「Nuôi SIM」頁面／「Hiệu chuẩn」欄位），而且 config.yaml 的 `keepalive.enabled` 必須為 true，否則不會發送任何訊息。每天在 `keepalive.run_hour` 執行；`failed`／`skipped` 的結果會顯示在「Cảnh báo」頁面並發送到該 SIM 的 Webhook。設定項：`keepalive.enabled`、`keepalive.interval_days`、`keepalive.max_per_month`、`keepalive.message`（`{{.Date}}` = yyyy-mm-dd）、`keepalive.run_hour`。
- **自動查詢門號（tự đọc số thuê bao）**：當 SIM 沒有門號（探測後 / 換卡後）時，smsie 會依電信商送出 `phone_lookup.codes` 中的 USSD 碼（Viettel `*098#`、Vinaphone `*110#`、Mobifone `*0#`、Vietnamobile `*102#`，依電信商名稱比對），解碼 `+CUSD` 回覆（UCS2/GSM7 十六進位）並儲存門號；每次變更都記錄在 `phone_number_histories`（`ussd` / `manual`）。手動觸發：卡槽卡片上的「Đọc số」或 `POST /api/v1/modems/:iccid/phone-lookup`（需 AT 指令權限）；歷史：`GET /api/v1/modems/:iccid/phone-history`。設定項：`phone_lookup.enabled`、`phone_lookup.codes`。
- **操作稽核紀錄（nhật ký hành động）**：所有需登入路由的 POST/PATCH/PUT/DELETE 都在處理完成後記錄（誰、動作代碼如 `sms.send` / `call.dial` / `user.delete`、SIM、目標號碼、HTTP 狀態、IP、請求內容前 4 KB 並移除 password/secret/key/token 欄位），另有系統列 `backup.ok` / `backup.failed` / `restore.applied`。管理員頁面「Nhật ký」（可依 SIM、使用者、動作、日期篩選）與 `GET /api/v1/audit`。超過 `audit.keep_days`（預設 365）天的紀錄會在啟動時及每日清除。
- **月報表（báo cáo tháng）**：每張 SIM 一列 — 收到 / 送出 / 失敗的 SMS（失敗 = 狀態 ≥ 400 的 `sms.send` 稽核列）、通話數與秒數、月初 / 月底餘額及差額、卡槽事件、保活發送數、警示數 — 加上合計。「Báo cáo」頁面可選月份並匯出 CSV（UTF-8 BOM，Excel 可直接開啟）；`GET /api/v1/reports/monthly?month=YYYY-MM&format=json|csv`，只包含呼叫者可檢視的 SIM。
- **自動備份 + 還原（僅 SQLite）**：每天在 `backup.hour`（預設 03:00，啟動時不執行）執行 `VACUUM INTO backups/smsie-YYYYMMDD-HHMMSS.db`，以 `PRAGMA integrity_check` 驗證（失敗 → 稽核 `backup.failed` 並通知所有 Webhook），只保留最新 `backup.keep` 份（預設 14）。「Báo cáo → Sao lưu」可列出、下載並「Sao lưu ngay」；API：`GET /api/v1/admin/backups`、`POST /api/v1/admin/backups/run`、`GET /api/v1/admin/backups/:name`。**還原絕不會在執行中直接覆寫資料庫**：「Khôi phục」（選擇既有備份或上傳 ≤ 512 MB 的 `.db`）或 `POST /api/v1/admin/restore` 會檢查檔頭與完整性，把檔案暫存為 `<dsn>.restore-pending` 並要求重新啟動；下次啟動時 smsie 先把目前資料庫快照到 `backups/pre-restore-*.db`，再換入新檔並記錄 `restore.applied`。在 Windows 服務下（`SMSIE_SERVICE=1`，由 `deploy/windows/smsie-service.xml` 設定）程式會在 2 秒後以代碼 3 結束，WinSW 自動重啟；否則需手動重啟。`deploy/windows/restore-backup.ps1` 使用同一個暫存機制。設定項：`backup.enabled`、`backup.dir`、`backup.hour`、`backup.keep`。
- **SMS 功能**
  - 可分頁、搜尋與檢視收到的簡訊。
  - 支援以 PDU 格式發送簡訊。
  - 收到 `+CMTI` 後可立即觸發掃描。
