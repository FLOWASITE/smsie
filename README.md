...
- **SIM health**: flags SIMs that look dead — silent (no received SMS for `sim_health.no_sms_days`, default 30, only while in a bay), unregistered (modem online but not registered on the network for `sim_health.unregistered_hours`, default 24) or absent (removed from its bay for `sim_health.absent_days`, default 7). Runs on the same daily scheduler as balance checks; findings show up on the *Cảnh báo* page, as a red dot on the bay card and in the maintenance list, and are sent to the SIM's webhooks then reminded every `sim_health.remind_days` (default 7) while the condition persists. Config keys: `sim_health.enabled`, `sim_health.no_sms_days`, `sim_health.unregistered_hours`, `sim_health.absent_days`, `sim_health.remind_days`.
- **Keep-alive (nuôi SIM)**: OFF by default and it COSTS MONEY — when a SIM has had no sent/received SMS for `keepalive.interval_days` (default 25; per-SIM override `keepalive_interval`), smsie sends one PAID intra-network SMS from it to another SIM of the same operator in the tray (never outside the tray), capped at `keepalive.max_per_month` (default 3) per SIM per calendar month. Each SIM must be opted in (`keepalive_enabled` in the modem profile, or the *Nuôi SIM* page / *Hiệu chuẩn* column) AND `keepalive.enabled` must be true in config.yaml, otherwise nothing is sent. Runs daily at `keepalive.run_hour`; `failed`/`skipped` runs show up on the *Cảnh báo* page and go to the SIM's webhooks. Config keys: `keepalive.enabled`, `keepalive.interval_days`, `keepalive.max_per_month`, `keepalive.message` (`{{.Date}}` = yyyy-mm-dd), `keepalive.run_hour`.
- **Phone-number lookup (tự đọc số thuê bao)**: when a SIM has no phone number (after probe / SIM swap) smsie sends the operator's USSD code from `phone_lookup.codes` (Viettel `*098#`, Vinaphone `*110#`, Mobifone `*0#`, Vietnamobile `*102#`; matched by operator name), decodes the `+CUSD` reply (UCS2/GSM7 hex) and stores the number; every change is kept in `phone_number_histories` (`ussd` / `manual`). Manual trigger: *Đọc số* on the bay card or `POST /api/v1/modems/:iccid/phone-lookup` (send-AT permission); history: `GET /api/v1/modems/:iccid/phone-history`. Config keys: `phone_lookup.enabled`, `phone_lookup.codes`.
- **Audit trail (nhật ký hành động)**: every POST/PATCH/PUT/DELETE on authenticated routes is recorded after the handler runs (who, action code such as `sms.send` / `call.dial` / `user.delete`, SIM, target number, HTTP status, IP, first 4 KB of the body with password/secret/key/token fields removed), plus system rows `backup.ok` / `backup.failed` / `restore.applied`. Admin page *Nhật ký* (filters by SIM, user, action, date) and `GET /api/v1/audit`. Rows older than `audit.keep_days` (default 365) are pruned at start-up and daily.
- **Monthly report (báo cáo tháng)**: one row per SIM — SMS received / sent / failed (failed = `sms.send` audit rows with status ≥ 400), calls and seconds, balance at start / end of month with delta, slot events, keep-alive sends, alerts — plus totals. *Báo cáo* page with month picker and CSV export (UTF-8 BOM, opens in Excel); `GET /api/v1/reports/monthly?month=YYYY-MM&format=json|csv`, limited to the SIMs the caller may view.
- **Automatic backup + restore (SQLite only)**: every day at `backup.hour` (default 03:00, never at boot) smsie runs `VACUUM INTO backups/smsie-YYYYMMDD-HHMMSS.db`, verifies it with `PRAGMA integrity_check` (failure → audit `backup.failed` + a message to every webhook) and keeps only the newest `backup.keep` (default 14). *Báo cáo → Sao lưu* lists, downloads and has *Sao lưu ngay*; API `GET /api/v1/admin/backups`, `POST /api/v1/admin/backups/run`, `GET /api/v1/admin/backups/:name`. **Restore never touches the live DB while running**: *Khôi phục* (pick a backup or upload a `.db` ≤ 512 MB) or `POST /api/v1/admin/restore` checks the header + integrity, stages the file as `<dsn>.restore-pending` and asks for a restart; at the next start smsie snapshots the current DB to `backups/pre-restore-*.db`, swaps the file in and audits `restore.applied`. Under the Windows service (`SMSIE_SERVICE=1`, set by `deploy/windows/smsie-service.xml`) the app exits with code 3 after 2 s and WinSW restarts it automatically; otherwise restart by hand. `deploy/windows/restore-backup.ps1` uses the same staging path. Config keys: `backup.enabled`, `backup.dir`, `backup.hour`, `backup.keep`.
- **SMS Operations**:
